...
//        val output = formatter.format(parser.parse("2018-12-14T09:55:00"))
    ...
    fun getLoadStatus(): String {
        val status = if (loadStatusId != null)
            LoadStatus.fromId(loadStatusId!!.toInt()) ?: LoadStatus.New_Load
        else LoadStatus.New_Load
        return if (status == LoadStatus.Completed) {
            StringUtil.getString(R.string.completed)
        } else if (status == LoadStatus.Cancelled) {
            StringUtil.getString(R.string.cancelled)
        } else {
